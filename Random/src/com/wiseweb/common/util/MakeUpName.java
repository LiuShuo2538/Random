...
public class MakeUpName {
    ...
    public static Object getModelName(Object value) {
        ...
        //名字库
        String[] fiName= {"修","宵","晨","戏","施","顺","祥","才","聪","银","戈","喻","声","生","铁","如","宸","誓","胜","挚","靖","齐",
        ...
        //判断传参是否合法
        boolean isRight = value instanceof String;
        System.out.println("判断传参是否合法"+isRight);
        //转换类型
        if (isRight){
            str = (String) value;
        }else {
            System.out.println("参数不合法，应该结束流程");
        }
        //判断是否含有特殊字符
        ...
        if (!isSpecial){
            ...
            if (isRareName){
                for (int i=0;i<lastName.length();i++){
                    firstNameHashCode = Math.abs(lastName.substring(i,i+1).hashCode());
                    newName+=fiName[firstNameHashCode%(fiName.length-1)];
                }
                newName+="·";
                for (int i=0;i<firstName.length();i++){
                    firstNameHashCode = Math.abs(firstName.substring(i,i+1).hashCode());
                    newName+=fiName[firstNameHashCode%(fiName.length-1)];
                }
            }else{
                lastNameHashCode = Math.abs(lastName.hashCode());//姓取正数
...
//        Pattern pattern = Pattern.compile("[`~!@#$%^&*()+=|{}':;',\\\\[\\\\].<>/?~！@#￥%……&*（）——+|{}【】‘；：”“’。，、？]");
            ...
            if (isTwoName){
                for (int i=0;i<nameArray.length;i++){
                    ...
                    if (matches1&&temp==1){
                        ...
                        firstNameHashCode = Math.abs(firstName.hashCode());//名取正数
     ...
     * @return
     */
    public static Object getRandomName(Object value) {
        ...
        if (!isSpecial){
            ...
            if (isRareName){
                for (int i=0;i<lastName.length();i++){
                    firstNameHashCode = Math.abs(lastName.substring(i,i+1).hashCode());
                    newName+=fiName[random.nextInt(firstNameHashCode%(fiName.length-1))];
                }
                newName+="·";
                for (int i=0;i<firstName.length();i++){
                    firstNameHashCode = Math.abs(firstName.substring(i,i+1).hashCode());
                ...
                lastNameHashCode = Math.abs(lastName.hashCode());//姓取正数
                newName = laName[random.nextInt(lastNameHashCode%(laName.length-1))]; //获得一个固定的姓氏
                if (isTwoName){
                    newName = contain[random.nextInt(lastNameHashCode%(contain.length-1))]; //获得一个固定的姓氏
                }
                for (int i=0;i<firstName.length();i++){
                    firstNameHashCode = Math.abs(firstName.substring(i,i+1).hashCode());
                    newName+=fiName[random.nextInt(firstNameHashCode%(fiName.length-1))];
            ...
            if (isTwoName){
                for (int i=0;i<nameArray.length;i++){
                    ...
                    if (matches1&&temp==1){
                        firstName=nameArray[i];
                        firstNameHashCode = Math.abs(firstName.hashCode());//名取正数
                        newName = fiName[random.nextInt(firstNameHashCode%(fiName.length-1))]; //获得一个固定的姓名字
                        nameArray[i]=newName.substring(0);
                    }
            ...
            if(isRareName){
                ...
                for (int i=0;i<nameArray.length;i++){
                    ...
                    if (matches1){
                        firstName=nameArray[i];
                        firstNameHashCode = Math.abs(firstName.hashCode());//名取正数
                        nameArray[i] = fiName[random.nextInt(firstNameHashCode%(fiName.length-1))]; //获得一个固定的姓名字
                    }
                }
            }
